...
<script src="{{asset('assets/js/product/add.js')}}"></script>
<script src="{{asset('assets/js/product/delete.js')}}"></script>
...
<script type="text/javascript">
        ...
        buttons: [
            ...
            },
            {
                ...
                action: function (e, dt, node, config) {
                    ...
                    $('#products-table thead tr:eq(1) th input').val('');
                    dt.draw();
                }
            }
        ],
    ...
    $('.edit-btn').on('click', function () {
        ...
        $.ajax({
            url: '{{ route("maintenance.product.edit", ":productId") }}'.replace(':productId', productId),
            type: 'GET',
            success: function (response) {
                $('#editProduct .modal-form').html(response);
                $('#editProduct').show();
...
@section('content')
<h4 class="mb-4">
    <span class="text-muted fw-light">BIOS /</span> Products
</h4>
...
<div class="card">
    <div class="card-header d-flex justify-content-between">
        ...
        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addProduct">
            <span><i class="ti ti-plus me-0 me-sm-1 ti-xs"></i>Add New Product</span>
        </button>
        @endcan
    </div>
    <div class="card-body">
                    ...
                    <tr>
                        <td>
                            <div class="d-flex align-items-center justify-content-center">
                                @can('edit products')
                                <a href="javascript:;" class="text-body edit-btn" data-record-id="{{ $product->id }}"
                                    data-bs-toggle="modal" data-bs-target="#editProduct"><i
                                        class="ti ti-edit ti-sm mx-2 text-info"></i></a>
                                @endcan
                                {{-- @can('delete products')
                                <form action="{{ route('maintenance.product.destroy', $product) }}" method="post"
                                    id="delete-product-form-{{ $product->id }}">
                                    @csrf
                                    @method('delete')
                                    <a href="javascript:;" onclick="confirmProductDelete({{ $product->id }})">
                        ...
                        <td>{{ $product->addedByUser->name }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
...
@include('_partials/_modals/product/add')

<!-- Edit Modal -->
<div class="modal" id="editProduct" tabindex="-1" aria-hidden="true" style="display: none">
